...
* [`src/cljs/nuds_reframe/`](src/cljs/nuds_reframe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/nuds_reframe/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `nuds-reframe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/nuds_reframe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns nuds-reframe.example
  (:require [nuds-reframe.config :as config])
