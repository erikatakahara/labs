...
* [`src/cljs/re_frame_study/`](src/cljs/re_frame_study/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/re_frame_study/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `re-frame-study` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_study/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-study.example
  (:require [re-frame-study.config :as config])
